...
public class ObservationTable implements DistinguishingSequenceGenerator, AccessSequenceGenerator {
    ...
    public Optional<Word<String>> checkForClosed() {
        ...
        Set<ArrayList<String>> states = new HashSet<>();
        for (Word<String> s : S) {
            states.add(table.get(s));
        }
        for (Word<String> s : S) {
            for (String a : this.inputSymbols) {
                Word<String> joined = s.append(a);
                if (!states.contains(table.get(joined))) {
    ...
    public Optional<Word<String>> checkForConsistent() {
        ...
        Map<ArrayList<String>, Word<String>> stateToS = new HashMap<>();
        for (Word<String> s : S) {
            ArrayList<String> row = table.get(s);

            if (stateToS.containsKey(row)) {
                Word<String> otherS = stateToS.get(row);
                for (String a : this.inputSymbols) {
                    ArrayList<String> rowSA = table.get(s.append(a));
                    ArrayList<String> rowSOtherA = table.get(otherS.append(a));
                    for (int i = 0; i < rowSA.size(); i++) {
                        if (!rowSA.get(i).equals(rowSOtherA.get(i))) {
                            System.out.printf("Row '%s' with suffix '%s' is not equal to row '%s'\n", s, a, otherS);
                            return Optional.of(new Word<>(a).append(E.get(i)));
                        }
                    }
                }
            } else {
     ...
     * consistent and closed. For any inconsistencies the model will create dummy
     * states.
     *
    ...
    public List<Word<String>> getAccessSequences() {
        ...
        List<Word<String>> access = new ArrayList<>();
        for(Word<String> s: S){
            if(rows.add(table.get(s))) {
                access.add(s);
            }
        }
        return access;
